...
pub trait Resource<'a, V: Vocabulary>: Clone {
	type Iris: 'a + IteratorWith<V, Item = V::Iri>;
	// type Blanks: 'a + Iterator<Item = V::BlankId>;
	type Literals: 'a + IteratorWith<V, Item = V::Literal>;
...
pub trait Interpretation<'a, V: Vocabulary>: Clone {
	...
	fn term_interpretation(&self, vocabulary: &mut V, term: uninterpreted::Term<V>) -> Result<Option<Id>, Self::Error> {
		match term {
			uninterpreted::Term::<V>::Id(rdf_types::Id::Iri(iri)) => self.iri_interpretation(vocabulary, iri),
	...
	fn terms_of(&self, id: Id) -> Result<OptionalResourceTerms<'a, V, Self::Resource>, Self::Error> {
		match self.get(id)? {
